...
use bellperson::groth16::{
    create_random_proof, create_random_proof_batch_with_type, generate_random_parameters,
    prepare_verifying_key, verify_proof, verify_proofs_batch, BellTaskType, Proof,
};
...
fn test_mimc(task_type: Option<BellTaskType>) {
    ...
    let proving_batch = Instant::now();
    {
        ...
        // Create a groth16 proof with our parameters.
        let proofs =
            create_random_proof_batch_with_type(vec![c; SAMPLES as usize], &params, rng, task_type)
                .unwrap();
    ...
    let handlers = (0..num_threads)
        .map(|i| {
            std::thread::spawn(move || {
                let task_type = if i == 0 {
                    Some(BellTaskType::WindowPost)
